...
class clienteParticular {
  ...
  calculaSubtotal() {
    this._subtotal = reservas.reduce(
      (acc, { tipoHabitacion, noches, pax, desayuno }) => 
        acc + (noches * this.calcularPrecioHabitacion(tipoHabitacion)) + this.calcularRecargoPersonaExtra(pax) + this.hasDesayuno(desayuno, pax)  , 0)
  }

  calcularTotal() {
    this._total = reservas.reduce(
      (acc, { tipoHabitacion, noches, desayuno, pax }) => 
        acc + noches * (this.calcularPrecioHabitacion(tipoHabitacion) + this.hasDesayuno(desayuno, pax)) * this._iva, 0)
  }
  ...
  hasDesayuno(desayuno, pax){
    return desayuno ? pax * 15 : 0;
  } 
...
class tourOperador extends clienteParticular{ 
  calcularTotal() {
    this._total = reservas.reduce(
      (acc, { tipoHabitacion, noches, desayuno, pax }) => 
      acc + noches * (this.calcularPrecioHabitacion(tipoHabitacion) + this.hasDesayuno(desayuno, pax)) * this._iva * 0.85, 0)
  }
